...
if(isset($_SESSION['UsuarioLogado'])){
	if ($_SESSION['UsuarioLogado'] == true) {
		echo "<script>alert('Vc ja esta logado.')</script>";
	}
}
...
$teste = DBQuery('*', 'usuario', "WHERE username = '$username' AND senha = '$senha'");
$userr = $teste->fetch_array();   

# Se existir encaminha para o painel, se não mostra erro de login.
if (mysqli_num_rows($teste) > 0) {
    $_SESSION['UsuarioLogado'] = true;
    $_SESSION['User'] = $username;
    $_SESSION['UserId'] = $userr['id'];
    $_SESSION['adminTrue'] = $userr['admin']; 
...
<body>
    ...
    <form action="" method="POST">
        ...
        <input type="text" placeholder="Email ou User" name="username" required>

        <label for="password">Senha</label>
        <input type="password" placeholder="Senha" name="senha" required >

        <button name="entrar" value="Entrar">Logar</button>
<!--        <div class="social">
